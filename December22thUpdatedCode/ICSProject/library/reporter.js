...
generateSubReport: function (csv) {
    ...
    captureScreenShot: function (png, currentStep) {
       /* var path = './results/ICSFLReport/' + currentStep + '.png';
       storeScreenShot(png, path);*/
       var jpath = './results/ICSFLReport/'+global.SUITENAME+'-'+global.TIMESTAMP+'/' + currentStep + '.png';
    ...
    appendTest: function (TestStep, TestStepDesc, Result) {
        ...
        if (Result.indexOf('FAIL') > -1) {            
            ...
            fs.appendFile('./results/ICSFLReport/report_assets/CSV_FailedTestList.csv',dt, function (err) {
                if (err) throw err;
            });
            browser.takeScreenshot().then(function (png) {
                var rand = Math.floor((Math.random() * 10000) + 1);
                var ScrShotFileName = 'Scr_' + rand + '_' + currentTimeStamp();
                var jpath = './results/ICSFLReport/'+global.SUITENAME+'-'+global.TIMESTAMP+'/' + ScrShotFileName + '.png';
                storeScreenShot(png, jpath);
    ...
    // console.log('*******csv*******' + csv);
    fs.readFile('./results/ICSFLReport/report_assets/' + csv + '.csv', 'utf8', function (error, data) {
        if (error) {
        ...
        fs.appendFile('./results/ICSFLReport/report_assets/CSV_Merge_' + Browser_Name + '.csv', data, function (err) {
            if (err) throw err;
...
function append_data_into_csv(data, file_name) {
    fs.appendFile('./results/ICSFLReport/report_assets/' + file_name + '.csv', data, function (err) {
        if (err) throw err;
        console.log(data);
    });
...
function detailedReport(csv, runTimeDisplayFlag) {
    report_name = "details_result_report_" + csv + "_" + timeStamp + ".htm";
    fs.readFile('./results/ICSFLReport/report_assets/Template_Details_Results.html', function (err, data) {
        if (err) {
            console.log(err);
            throw err;
        }
        jsdom.env({
            ...
            scripts: ['./library/jquery-2.2.4.min.js'],
                ...
                var lineRdr = require("readline").createInterface({
                    input: fs.createReadStream('./results/ICSFLReport/report_assets/' + csv + '.csv')
                });
                Sr_NO = 0;
                Failed = 0;
                var line_arr;
                lineRdr.on('line', function (line) {
                    ...
                    var tcname = "\"#" + global.current_TestCase + "\"";
                ...
                lineRdr.on('close', function () {
                    ...
                    Failed = 0;
                       /* fs.writeFile("./results/ICSFLReport/" + report_name, window.document.documentElement.outerHTML
                            , function (error) {
                                if (error) throw error;
                            });*/
                            fs.writeFile("./results/ICSFLReport/"+global.SUITENAME+'-'+global.TIMESTAMP+"/" + report_name, window.document.documentElement.outerHTML
                                , function (error) {
                                    if (error) throw error;
                                });
...
function summaryReport(csv, fn) {
    return new Promise(function (resolve, reject) {
        pushSumData(csv).then(function () {
            ...
            fs.readFile('./results/ICSFLReport/report_assets/Template_Summary_Results.html', function (err, data) {
                if (err) {
                    throw err;
                }
                jsdom.env({
                    ...
                    scripts: ['./library/jquery-2.2.4.min.js'],
                        ...
                        $.each( csvData, function( key, value ) {
                            ...
                            fs.writeFile("./results/ICSFLReport/"+global.SUITENAME+'-'+global.TIMESTAMP+"/" + Jsum_report_name, window.document.documentElement.outerHTML,
                                function (error) {
                                    if (error) throw error;
                                });
                        });                       
...
function emailSumReport(csv, fn, email_content) {
    return new Promise(function (resolve, reject) {
        fs.readFile('./results/ICSFLReport/report_assets/JSumEmailResult.html', function (err, data) {
            if (err) {
                throw err;
            }
            jsdom.env({
                ...
                scripts: ['./library/jquery-2.2.4.min.js'],
                    ...
                    fs.writeFile("./results/ICSFLReport/"+global.SUITENAME+'-'+global.TIMESTAMP+"/" + Jsum_emailreport, window.document.documentElement.outerHTML,
                        function (error) {
                            if (error) throw error;
                        });
...
function elapsed_time_of_each_TC(TC_ID, JSON_FILE) {
    var data = require('../results/ICSFLReport/report_assets/' + JSON_FILE + '.json');
    var elapsedTime = 0;
    var stat;
...
function cleaningCSV(csv) {
    fs.truncate('./results/ICSFLReport/report_assets/' + csv + '.csv', 0, function () {
        console.log('done')
    })
    fs.truncate('./results/ICSFLReport/report_assets/CSV_FailedTestList.csv', 0, function () {
        console.log('done')
    });
}
